...
<cfcomponent name="RemoteFactoryBean" 
	...
	<cfset variables.useAnyTypeForRemote = true />	
	<cfset variables.useJsonReturnType = true />	
	...
	<cffunction name="setUseAnyTypeForRemote" access="public" returntype="void" output="false">
		<cfargument name="useAnyTypeForRemote" type="boolean" required="true" />
		<cfset variables.useAnyTypeForRemote = arguments.useAnyTypeForRemote />
	</cffunction>
	<cffunction name="setUseJsonReturnType" access="public" returntype="void" output="false">
		<cfargument name="useJsonReturnType" type="boolean" required="true" />
		<cfset variables.useJsonReturnType = arguments.useJsonReturnType />
	</cffunction>
					...
					<cfif not ListFindNoCase('init', functionName) and not StructKeyExists(usedFunctions, functionName)>
						...
						<cfif methodPointcutAdvisor.matches(functionName)>
							...
							<cfset functionString = functionString & 
								   variables.aopProxyUtils.createRemoteMethod(md.functions[functionIx], functionName, 'remote',variables.useAnyTypeForRemote,variables.useJsonReturnType)  & Chr(10) & Chr(10) />
