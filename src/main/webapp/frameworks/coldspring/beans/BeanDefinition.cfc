...
<cfcomponent name="BeanDefinition" 
	...
	<cffunction name="addConstructorArg" access="public" output="false" returntype="void"  
		...
		<cfset var name = "" />
		<cfif Len(arguments.constructorArg.getName())>
			<cfset name = arguments.constructorArg.getName() />
		<cfelse>
			<cfset name = StructCount(variables.instanceData.constructorArgs) + 1 />
			<cfset arguments.constructorArg.setName(name) />
		</cfif>
		<cfset variables.instanceData.constructorArgs[name] = arguments.constructorArg />
	</cffunction>
